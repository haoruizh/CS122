...
// only function in this example 
// that is dealing with a doubly linked list!
// precondition: list must not be empty! n > 0
int deleteFront(Node **pList, Data *pData)
{
	...
	*pList = (*pList)->pNext;
	...
	if (*pList == NULL)
	{
		// there's only 1 node in list

		//free(pTemp);
		*pData = pTemp->item; // struct assignment
		free(pTemp);
		//pData = &(pTemp->item);
		//free(pTemp->pNext);
		//free(pTemp->pPrev);
