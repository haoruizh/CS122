...
{
    ListNode * pCur, *cCur, *pNew;
	// fill in your code here!
    if(copyList.mpHead!=nullptr)
    {
        cCur = copyList.mpHead;
        while (cCur != nullptr)
        {
            pNew = new ListNode(cCur->getData());
            if (mpHead == nullptr) //only node
            {
...
List & List::operator= (const List &rhs) // overloaded assignment operator - must perform a deep copy
{
    ...
    if (rhs.mpHead != nullptr)
    {
        ...
        {
            pNew = new ListNode(cCur->getData());
            if (this->mpHead == nullptr) //only node
            {
...
bool List::insertInOrder(const int newData)     
{
	...
	// fill in your code here!
    ListNode *pCur, *pNew, *pNext;
    pNew = new ListNode(newData);
    if (this->mpHead != nullptr)
    {
        pCur = mpHead;
        pNext = pCur->getNextPtr();
        while (pNext != nullptr &&pNext->getData()<pNew->getData())
        {
            pNext = pNext->getNextPtr();
            pCur = pCur->getNextPtr();
        }
        pNew->setNextPtr(pNext);
        pCur->setNextPtr(pNew);
        success = true;
    }
